...
Route::get('/download-test-image', function () {
    // 1. Database se 5 products uthao
    $products = Product::active()->inRandomOrder()->limit(5)->pluck('name');

    if ($products->isEmpty()) {
        return "Error: Database mein products nahi hain!";
    }

    // 2. Image Canvas Banao (Width: 600, Height: 400)
    $width = 600;
    $height = 400;
    $image = imagecreate($width, $height);

    // 3. Colors Define karo
    $white = imagecolorallocate($image, 255, 255, 255); // Background
    $black = imagecolorallocate($image, 0, 0, 0);       // Text Color

    // 4. Heading Likho
    // Font: 5 (Built-in largest font), X: 200, Y: 20
    imagestring($image, 5, 200, 20, "BULK ORDER LIST", $black);

    // 5. Products List Likho
    $y = 70; // Y position start
    foreach ($products as $name) {
        // Product Name draw karo
        imagestring($image, 5, 50, $y, "- " . $name, $black);
        $y += 50; // Agli line ke liye gap
    }

    // 6. Image Download Karwao
    header('Content-Type: image/png');
    header('Content-Disposition: attachment; filename="test_ocr_image.png"');
    imagepng($image);
    imagedestroy($image);
});
...
Route::get('/download-test-pdf', function () {
    ...
    // 2. HTML banao jo PDF mein dikhega
    $html = '
    <html>
    <head>
        <style>
            ...
            .item { padding: 10px; border-bottom: 1px solid #ddd; font-size: 14px; }
        </style>
    </head>
    <body>
        <h2>Bulk Order Product List</h2>
        <p>Upload this file to test bulk order feature.</p>
        <div class="list">';
    ...
    // 3. PDF Generate aur Download karo
    $pdf = Pdf::loadHTML($html);
    return $pdf->download('bulk_order_test.pdf');
});

Route::get('/download-test-file', function () {
    // 1. Aapke DB se 5 random active products uthayega
    $products = Product::active()->inRandomOrder()->limit(5)->get();

    if ($products->isEmpty()) {
        return "Error: Aapke Database mein koi Active Product nahi hai!";
    }

    // 2. Data format karega
    $data = [];
    foreach ($products as $product) {
        $data[] = [
            'name' => $product->name,  // Ye column Import ke liye zaroori hai
            'price' => $product->unit_price, // Ye sirf dekhne ke liye
            'id_check' => $product->id // Ye verify karne ke liye
        ];
    }

    // 3. Excel Download karega using FastExcel
    return (new FastExcel($data))->download('bulk_upload_test.xlsx');
});

Route::get('/test-ocr', function () {
    $imagePath = public_path('test_ocr.png');

    // Ye line screen par path print karegi
    dd("Laravel is looking here: " . $imagePath);

    if (!file_exists($imagePath)) {
        return "Error: Image nahi mili!";
    }
    // ... baaki code
});
...
Route::group(['namespace' => 'Web', 'middleware' => ['maintenance_mode', 'guestCheck']], function () {
    ...
    Route::controller(WebController::class)->group(function () {
        ...
        // Route::post('product-bulk-import', 'bulk_import_products')->name('product-bulk-import');
        Route::post('product-bulk-import', 'bulk_import_preview')->name('product-bulk-import');

        // 2. Preview ke baad Cart me add karne ke liye (New)
        Route::post('product-bulk-import-confirm', 'confirm_bulk_import')->name('product-bulk-import-confirm');
