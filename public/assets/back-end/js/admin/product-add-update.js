...
// update custom file label and show preview for variant images (also clear any removal flag)
$(document).on('change', '#sku_combination .custom-file-input', function() {
    var fileName = $(this).val().split('\\').pop();
    $(this).next('.custom-file-label').html(fileName);
    var input = this;
    var $td = $(input).closest('td');

    // if user chooses a new file, clear any previous "remove" flag
    $td.find('input[name^="remove_image_"]').val(0);
    // also remove existing cross icon if there to avoid duplicates
    $td.find('.remove-variant-image').remove();

    if (input.files && input.files[0]) {
        var reader = new FileReader();
        reader.onload = function(e){
            $td.find('.variant-image-preview').remove();
            // wrap image and cross inside container
            var html = '<div class="position-relative d-inline-block variant-image-wrapper" style="margin-top:5px;">'
                     + '<img src="'+e.target.result+'" class="img-fluid variant-image-preview" style="height:50px;"/>'
...
// handle click on cross icon to remove variant image
$(document).on('click', '.remove-variant-image', function(){
    var $td = $(this).closest('td');
    var $input = $td.find('.custom-file-input');
    var fieldName = $input.attr('name'); // e.g. image_color-red
    var hiddenName = 'remove_' + fieldName;

    // remove preview wrapper entirely
    var $wrapper = $(this).closest('.variant-image-wrapper');
    $wrapper.remove();

    // clear file input value
    $input.val('');
    $td.find('.custom-file-label').html('');

    // set or create hidden remove flag
    var $hidden = $td.find('input[name="'+hiddenName+'"]');
    if ($hidden.length) {
        $hidden.val(1);
    } else {
        $td.append('<input type="hidden" name="'+hiddenName+'" value="1" class="remove-image-input">');
    }
});
