...
class ProductService
{
    ...
    public function getVariations(object $request, array $combinations, object $product = null): array
    {
        $variations = [];
        if (isset($combinations[0]) && count($combinations[0]) > 0) {
            foreach ($combinations as $combination) {
                ...
                if ($request->hasFile('image_' . $slug)) {
                    $imageName = $this->upload('product/variant/', 'webp', $request->file('image_' . $slug));
                } elseif ($product && $product->variation) {
                    // retain existing image unless user indicated removal
                    $existingVariations = json_decode($product->variation, true);
                    if (is_array($existingVariations)) {
                        foreach ($existingVariations as $existingVariation) {
                            if (isset($existingVariation['type']) && $existingVariation['type'] == $str) {
                                $imageName = $existingVariation['image'] ?? null;
                ...
                // check if remove flag submitted
                if ($request->has('remove_image_' . $slug) && $request->input('remove_image_' . $slug)) {
                    // delete existing file from storage if present
                    if ($imageName) {
                        try {
                            $this->delete(filePath: '/product/variant/' . $imageName);
                        } catch (\Exception $e) {
    ...
    public function getUpdateProductData(object $request, object $product, string $updateBy): array
    {
        ...
        $combinations = $this->getCombinations($this->getOptions(request: $request));
        $variations = $this->getVariations(request: $request, combinations: $combinations, product: $product);
