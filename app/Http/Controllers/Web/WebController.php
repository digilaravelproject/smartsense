...
class WebController extends Controller
{
    ...
    public function bulk_import_preview(Request $request)
    {
        // 1. Validation
        $request->validate([
            'bulk_file' => 'required|mimes:xlsx,csv,xls,png,jpg,jpeg,pdf|max:10240', // Max 10MB
        ]);
        ...
        $productRows = []; // each item: ['name' => string, 'quantity' => int|null]
        $productQuantities = []; // mapping product_id => quantity (filled after matching)

        // Small helper to parse a text line and try to extract trailing quantity.
        $extractNameAndQty = function ($text) {
            $text = trim($text);
            $original = $text;

            // Remove common labels
            $text = preg_replace('/(?:Product Name:|Item:|Name:)\s*/i', '', $text);
            ...
            // 1) Look for explicit qty labels: qty, quantity, qto, qte, qtty
            if (preg_match('/(?:qty|quantity|qte|qto|qtty)[\s:\-]*([0-9,\.]+)/i', $text, $m)) {
                $qty = preg_replace('/[^0-9]/', '', $m[1]);
                $name = trim(preg_replace('/(?:qty|quantity|qte|qto|qtty)[\s:\-]*[0-9,\.]+/i', '', $text));
                return ['name' => $name ?: $original, 'quantity' => $qty !== '' ? intval($qty) : null];
            }

            // 2) Look for a trailing number at the end of the line
            if (preg_match('/([0-9]+(?:[,\.][0-9]+)*)\s*$/', $text, $m)) {
                $num = preg_replace('/[^0-9]/', '', $m[1]);
                if ($num !== '') {
                    $name = trim(preg_replace('/([0-9]+(?:[,\.][0-9]+)*)\s*$/', '', $text));
                    return ['name' => $name ?: $original, 'quantity' => intval($num)];
                }
            }

            // 3) As a last resort, pick the first standalone number in text (may be part of model numbers)
            if (preg_match('/\b([0-9]{1,6})\b/', $text, $m)) {
                $num = $m[1];
                $name = trim(str_replace($num, '', $text));
                return ['name' => $name ?: $original, 'quantity' => intval($num)];
            }

            return ['name' => $original, 'quantity' => null];
        };
        ...
        try {
            ...
            if (in_array($extension, ['xlsx', 'csv', 'xls'])) {
                ...
                foreach ($collections as $row) {
                    // Try to find 'name' column, else take the first column
                    $name = isset($row['name']) ? $row['name'] : (array_values($row)[0] ?? null);

                    // Try to find quantity from common column names or second column
                    $quantity = null;
                    if (isset($row['quantity'])) $quantity = $row['quantity'];
                    elseif (isset($row['qty'])) $quantity = $row['qty'];
                    else {
                        $vals = array_values($row);
                        if (count($vals) > 1) $quantity = $vals[1];
                    }

                    if ($name) {
                        $productRows[] = [
                            'name' => trim($name),
                            'quantity' => is_numeric($quantity) ? intval($quantity) : null,
                        ];
                    }
                }
            }
            ...
            elseif ($extension == 'pdf') {
                ...
                // Iterate lines with index so we can attach numeric-only lines
                $count = count($lines);
                for ($i = 0; $i < $count; $i++) {
                    $cleanLine = trim($lines[$i]);
                    if ($cleanLine === '') continue;

                    // Numeric-only line (e.g., a quantity placed on the next line)
                    if (preg_match('/^\s*([0-9]+(?:[,\.][0-9]+)*)\s*\.?\s*$/', $cleanLine, $m)) {
                        // Attach to previous product if any and previous has null/empty qty
                        $num = preg_replace('/[^0-9]/', '', $m[1]);
                        if (!empty($num) && count($productRows) > 0) {
                            $lastIndex = count($productRows) - 1;
                            if (empty($productRows[$lastIndex]['quantity'])) {
                                $productRows[$lastIndex]['quantity'] = intval($num);
                            } else {
                                // aggregate if quantity already set
                                $productRows[$lastIndex]['quantity'] = intval($productRows[$lastIndex]['quantity']) + intval($num);
                            }
                            continue;
                        }
                    }

                    // Filter: Ignore very short words
                    if (strlen($cleanLine) > 2) {
                        $parsed = $extractNameAndQty($cleanLine);
                        $productRows[] = ['name' => $parsed['name'], 'quantity' => $parsed['quantity']];
                    }
                }
            }
            // ----------------------------------------------------
            // CASE 3: Image/PDF (OCR) via OCR.space API (preferred)
            // ----------------------------------------------------
            elseif (in_array($extension, ['png', 'jpg', 'jpeg', 'pdf'])) {

                try {
                    // Use OCR.space API key from env (use 'helloworld' for limited testing)
                    $ocrKey = env('OCR_SPACE_API_KEY', 'helloworld');
                    $ocrEndpoint = 'https://api.ocr.space/parse/image';

                    // Prepare multipart request with file. Use the original client filename
                    // so OCR.space can validate extension correctly.
                    $clientFilename = $request->file('bulk_file')->getClientOriginalName();
                    $response = Http::attach('file', fopen($filePath, 'r'), $clientFilename)
                        ->asMultipart()
                        ->post($ocrEndpoint, [
                            'apikey' => $ocrKey,
                            'language' => 'eng',
                    ...
                    $fullText = null;
                    if (isset($result['IsErroredOnProcessing']) && $result['IsErroredOnProcessing'] == false && !empty($result['ParsedResults'][0]['ParsedText'])) {
                        $fullText = $result['ParsedResults'][0]['ParsedText'];
                    }

                    // If OCR.space didn't yield text, fallback to local Tesseract (if installed) or PDF parser
                    if (empty($fullText)) {
                        // Prefer local Tesseract only if the PHP wrapper class is available
                        if (class_exists('\\thiagoalessio\\TesseractOCR\\TesseractOCR')) {
                            try {
                                $fullText = (new \thiagoalessio\TesseractOCR\TesseractOCR($filePath))->run();
                            } catch (\Throwable $e) {
                                $fullText = null;
                            }
                        }

                        // If still empty and file is PDF, try the PDF parser as a last resort
                        if (empty($fullText) && $extension == 'pdf') {
                            try {
                                $parser = new PdfParser();
                                $pdf = $parser->parseFile($filePath);
                    ...
                    if (empty($fullText)) {
                        $err = $result['ErrorMessage'][0] ?? ($result['ErrorMessage'] ?? 'No_text_detected_in_image');
                        Toastr::warning('OCR.space: ' . (is_array($err) ? implode(' ', $err) : $err));
                        return back();
                    }

                    // Break into lines and parse
                    $lines = preg_split('/\r\n|\r|\n/', $fullText);

                    // Iterate lines and handle numeric-only quantity lines placed on separate rows
                    $count = count($lines);
                    for ($i = 0; $i < $count; $i++) {
                        $cleanLine = trim($lines[$i]);

                        // Clean unwanted symbols commonly found in OCR
                        $cleanLine = str_replace(['|', '_', '*', '•', '°', '- '], ' ', $cleanLine);
                        $cleanLine = trim($cleanLine);

                        if ($cleanLine === '') continue;

                        // If this line is numeric-only, try to attach to previous row
                        if (preg_match('/^\s*([0-9]+(?:[,\.][0-9]+)*)\s*\.?\s*$/', $cleanLine, $m)) {
                            $num = preg_replace('/[^0-9]/', '', $m[1]);
                            if (!empty($num) && count($productRows) > 0) {
                                $lastIndex = count($productRows) - 1;
                        ...
                        // Valid text check
                        if (strlen($cleanLine) > 2) {
                            // First, try simple token-based trailing number detection (covers "Name    10" cases)
                            $tokens = preg_split('/\s+/', $cleanLine);
                            $lastToken = end($tokens);
                            $numToken = preg_replace('/[^0-9]/', '', $lastToken);

                            if ($numToken !== '') {
                                // Build name from remaining tokens
                                $nameTokens = array_slice($tokens, 0, count($tokens) - 1);
                                $nameGuess = trim(implode(' ', $nameTokens));
                                if ($nameGuess === '') {
                                    // fallback to full line if nothing left
                                    $nameGuess = $cleanLine;
                                }
                                $productRows[] = ['name' => $nameGuess, 'quantity' => intval($numToken)];
                            } else {
                                $parsed = $extractNameAndQty($cleanLine);
                                $productRows[] = ['name' => $parsed['name'], 'quantity' => $parsed['quantity']];
                            }
                    ...
                    Toastr::error('System Error: ' . $e->getMessage());
                    return back();
                }
            }
        } catch (\Exception $e) {
            Toastr::error(translate('Error_reading_file') . ': ' . $e->getMessage());
            return back();
        }
        ...
        if (empty($productRows)) {
            Toastr::warning(translate('No_readable_text_found_in_file'));
            return back();
        }
        ...
        foreach ($productRows as $row) {
            $rawName = $row['name'];
            $rowQuantity = $row['quantity'] ?? null;

            // Basic cleaning: remove stray punctuation/characters that may break LIKE matching
            $searchName = preg_replace('/[^\p{L}\p{N}\s\-]+/u', '', $rawName);
            $searchName = trim(preg_replace('/\s+/', ' ', $searchName));
            if (strlen($searchName) < 3) continue;

            // Search in Database (Active Products Only)
            $product = \App\Models\Product::active()
                ->where('name', 'LIKE', "%{$searchName}%")
                ->with(['rating', 'seller.shop']) // Load relations for UI
                ->first();
            ...
            if ($product) {
                ...
                foreach ($foundProducts as $p) {
                    if ($p->id == $product->id) { $exists = true; break; }
                }

                // Determine quantity for this row (default to 1 when not provided)
                $qtyToAdd = ($rowQuantity && intval($rowQuantity) > 0) ? intval($rowQuantity) : 1;

                if (!$exists) {
                    $foundProducts[] = $product;

                    // Set initial quantity for this matched product
                    $productQuantities[$product->id] = $qtyToAdd;
                } else {
                    // If same product appears multiple times in the input, aggregate quantities
                    $productQuantities[$product->id] = ($productQuantities[$product->id] ?? 0) + $qtyToAdd;
                }
            } else {
                // Add to Not Found list (avoid duplicates)
                if (!in_array($searchName, $notFoundNames)) {
                    $notFoundNames[] = $searchName;
                }
            }
        }

        // Return Preview View (also pass raw parsed rows for debugging)
        return view('web-views.products.bulk-import-preview', compact('foundProducts', 'notFoundNames', 'productQuantities', 'productRows'));
    }

    public function confirm_bulk_import(Request $request)
    {
        $products = $request->input('products', []); // Array of ID and Qty

        if (count($products) == 0) {
            Toastr::warning(translate('No_products_selected'));
            return back();
        }
        ...
        foreach ($products as $item) {
            ...
            if (isset($item['selected']) && $item['selected'] == 1) {
                ...
                // Use CartManager logic
                $result = CartManager::add_to_cart($cartRequest);
        ...
        if ($addedCount > 0) {
            Toastr::success($addedCount . ' ' . translate('products_added_to_cart_successfully'));
            return redirect()->route('checkout-details');
        } else {
            Toastr::warning(translate('Could_not_add_products_to_cart._Please_check_stock.'));
            return redirect()->route('shop-cart');
        }
    }
