...
if (!function_exists('getStorageImages')) {
    function getStorageImages($path, $type = null, $source = null): string
    {
        if ($source && base_path($source)) {
            ...
            return (!empty($path) && is_array($path) && $path['status'] == 200) ? $path['path'] : dynamicAsset($source);
        }
        if ($source && file_exists($source)) {
            return (!empty($path) && is_array($path) && $path['status'] == 200) ? $path['path'] : $source;
        }

        // support legacy usage where only a filename string is passed (e.g. variant images)
        if (is_string($path) && !empty($path)) {
            // determine likely directory; variant images are stored under ``product/variant``
            $storageDir = 'product';
            if (file_exists(storage_path('app/public/product/variant/' . $path))) {
                $storageDir = 'product/variant';
            }
            $link = storageLink($storageDir, $path, config('filesystems.disks.default') ?? 'public');
            // storageLink always returns an array when called from here
            if (is_array($link)) {
                return $link['path'] ?? '';
            }
            // ensure string result (could be null if something odd happens)
            return (string) $link;
        }
