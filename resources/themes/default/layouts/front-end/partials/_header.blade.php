            ...
            <div class="container px-10px">
                <div class="collapse navbar-collapse text-align-direction" id="navbarCollapse">
                    ...
                    <ul class="navbar-nav">
                        ...
                        @elseif(count($web_config['featured_deals']) > 0 ||
                                ($web_config['flash_deals'] && count($web_config['flash_deals_products']) > 0) ||
                                $web_config['discount_product'] > 0 ||
                                $web_config['clearance_sale_product_count'] > 0)
                            <li class="nav-item">
                        ...
                        {{-- Bulk Upload Button --}}
                        <li class="nav-item dropdown">
                            <a class="nav-link text-capitalize" href="javascript:" data-toggle="modal"
                                data-target="#bulkUploadModal">
                                <i class="fa fa-upload mr-1"></i> {{ translate('Bulk_Order') }}
                            </a>
                        </li>
                    </ul>
                    @if (auth('customer')->check())
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ translate('Upload_Product_List') }}</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('product-bulk-import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <label>{{ translate('Upload_File') }} (Excel, CSV, Image, PDF)</label>
                        <div class="custom-file">
                            <input type="file" name="bulk_file" class="custom-file-input" id="bulkFile" accept=".xlsx, .csv, .xls, .png, .jpg, .jpeg, .pdf" required>
                            <label class="custom-file-label" for="bulkFile">{{ translate('Choose_file') }}</label>
                        </div>
                        <small class="text-muted">
                            * Excel/CSV: Column A should contain Product Names.<br>
                            * Image/PDF: Should be clear text (Requires OCR setup).
                        </small>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">{{ translate('Close') }}</button>
                    <button type="submit" class="btn btn-primary">{{ translate('Process_&_Checkout') }}</button>
                </div>
            </form>
        </div>
    </div>
