@php
    ...
    if (!empty($product->images_full_url) && isset($product->images_full_url[1])) {
        // Assuming images_full_url is an array of image path strings
        $hoverImageUrl = $product->images_full_url[1];
    }
@endphp

<div class="card-product style-4 h-100">
    <div class="card-product-wrapper radius-16 line-2 asp-ratio-0">
        ...
        <ul class="list-product-btn">
            ...
            <li>
                <!-- Compare functionality might need a specific implementation -->
            ...
            <span class="out_of_stock_badge">{{translate('out_of_stock')}}</span> <!-- New class for styling -->
            @endif
        </div>

    <div class="card-product-info text-center">
        @if($overallRating[0] != 0)
            <div class="rating-show justify-content-center mb-1"> <!-- Centered rating -->
        ...
        @php
            ...
            // Fallback or alternative logic if you have specific product attributes for tags
            // Example: if ($product->attributes && count($tags) < 2) { ... }
        @endphp
        ...
        @else
            <!-- Optional: Add a spacer if no tags to maintain consistent card height -->
            <div style="height: 29px;" class="mt-2"></div>
                    @endif
    </div>
</div>
